...
def get_model_pipeline(model, feats):
    ...
    if not isinstance(model, (LogisticRegression, XGBClassifier)):
        raise ValueError(
            "Model should be either XGBClassifier or LogisticRegression.")
    ...
    for key in required_keys:
        if key not in feats:
            raise KeyError(
                f"Missing required key: '{key}' in feats dictionary.")

    # Preprocessing for categorical features
    if isinstance(model, XGBClassifier):
        encoder = OrdinalEncoder(
            handle_unknown='use_encoded_value', unknown_value=1000)
...
def train_model(model, X_train, y_train, param_grid, scoring='accuracy', cv=3):
    ...
    grid_search = GridSearchCV(
        estimator=model, param_grid=param_grid, cv=cv, scoring=scoring, verbose=1)
